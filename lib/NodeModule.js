...
function NodeModule_loadPackageJSON(file,           // @arg PathString: package.json file path.
                                    packageFiles) { // @arg Object(= null): { package.json: JSON, ... }
                                                    ...
                                                    // @help: NodeModule.loadPackageJSON
    if (packageFiles) {
...
function NodeModule_validatePackageJSON(file,  // @arg PathString: package.json file path.
                                        pkg) { // @arg Object: package.json object.
                                               ...
                                               // @help: NodeModule.validatePackageJSON
    var name = pkg["name"];
...
function NodeModule_removeDevToolModules(packageData) { // @arg Object: NodeModule.collectPackageData() result value
                                                        ...
                                                        // @help: NodeModule.removeDevToolModules
...
function _sortModuleListByDependencyOrder(list,   // @arg ModuleNameStringArray: [moduleName, ...]
    ...
    function _sortFunction(moduleA,   // @arg ModuleNameString: module A
                           moduleB) { // @arg ModuleNameString: module B
                                      // @ret Integer: -1, 0, +1
        if (moduleA in tree && _isDependency(tree, moduleA, moduleB)) {
            return 1;  // [moduleA, moduleB].sort() -> [moduleB, moduleA]
        }
        if (moduleB in tree && _isDependency(tree, moduleB, moduleA)) {
    ...
    function _isDependency(tree,      // @arg Object: module dependency sub tree. { "uupaa.nodemodule.js": { ... }, ... }
                           ...
                           moduleB) { // @arg String: find target module. "uupaa.base64.js"
                                      ...
                                      // @recursive:
        if (moduleA in tree) {
            if (moduleB in tree[moduleA]) {
                return true;
            }
            for (var name in tree[moduleA]) { // { console, valid, help, task, test, watch, plato, minify }
                if (tree[name] && _isDependency(tree[name], name, moduleB)) {
                    return true;
                }
            }
        }
