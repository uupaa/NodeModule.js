...
function _collectModuleData(result,      // @arg Object: { nodeFiles, ..., devDependenciesModules }
    ...
    // merge(devDependenciesModules, dependenciesModule)
    if (develop) { // in develop
        if (json["dependencies"]) { // search dependencies modules
            Object.keys(json["dependencies"]).forEach(function(moduleName) {

                if (result["devDependenciesModules"].indexOf(moduleName) < 0) { // avoid duplicate
                    var path = dir + "node_modules/" + moduleName + "/" + fileName;

                    if (fs["existsSync"](path)) {
                        result["devDependenciesModules"].push(moduleName); // add devDependenciesModules
