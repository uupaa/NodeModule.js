...
NodeModule["verbose"] = true;

// --- implements ------------------------------------------
function NodeModule_files(param) { // @arg Object - { dir, ignore, develop, packageFiles }
                                   // @param.dir PathString = ""         - search base directory.
                                   // @param.ignore ModuleNameArray = [] - ignore module names. ["uupaa.watch.js", ...]
                                   // @param.develop Boolean = false     - develop mode.
                                   // @param.packageFiles Object = null  - package.json files for UnitTest.
...
function NodeModule_sortModuleListByDependencyOrder(list,   // @arg ModuleNameStringArray - [moduleName, ...]
    ...
    function _sortFunction(moduleA,   // @arg ModuleNameString: module A
                           moduleB) { // @arg ModuleNameString: module B
                                      // @ret Integer: -1, 0, +1
        if (verbose) { console.log(moduleA, "と", moduleB, "を比較します"); }

        var subtree = _getSubTree(tree, moduleA);

        if (subtree && _isDependency(subtree[moduleA], moduleB)) {
            if (verbose) { console.log(moduleA, "は", moduleB, "に依存しています(先に読み込みが必要です)"); }
            return 1;  // [moduleA, moduleB].sort() -> [moduleB, moduleA]
        }

        subtree = _getSubTree(tree, moduleB);

        if (subtree && _isDependency(subtree[moduleB], moduleA)) {
            if (verbose) { console.log(moduleB, "は", moduleA, "に依存しています(先に読み込みが必要です)"); }
            return -1; // [moduleB, moduleA].sort() -> [moduleA, moduleB]
        }
        if (verbose) { console.log(moduleA, "と", moduleB, "に依存関係はありません"); }

        // return 0; // stay position. [!! BUGGY !!]
        // Array#sort does not stable sort in IE, Chrome and AS3.
        // The code `return 0;` becomes the this bug(is not interoperability)
        //
        // return 0; を返すと多くの環境では正しく動作しません。
        // IE, Chrome, AS3 においては Array#sort は非安定ソートとして実装されています。
        //
    ...
    function _isDependency(subtree, module) { // @recursive
        if (subtree) {
            if (module in subtree) {
                return true;
            }
            for (var name in subtree) {
                if (_isDependency(subtree[name], module)) {
                    return true;
                }
            }
        }
    ...
    function _getSubTree(tree, module) { // @recursive
        for (var name in tree) {
            if (name === module) {
                return tree;
            }
            var result = _getSubTree(tree[name], module);
            if (result) {
                return result;
            }
        }
